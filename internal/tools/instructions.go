...
const Instructions = "chrome-pilot-mcp drives a Chrome browser on this machine over the Chrome DevTools Protocol: " +
	...
	"Two tools take work_dir, take_screenshot and screencast_start, and in both it is required with no default: " +
	"the absolute path of an existing directory you can read back (your session or working directory). " +
	"The screenshot is written under <work_dir>/screenshots/, and the GIF that screencast_stop produces under " +
	"<work_dir>/screencasts/ unless screencast_start was given a filePath; both results return the file's path. " +
