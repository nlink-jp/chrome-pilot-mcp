...
import (
	...
	"github.com/nlink-jp/chrome-pilot-mcp/internal/jsonrpc"
...
// to return multiple content blocks (image + text, etc.) — e.g.
// take_screenshot returning the saved path plus the image itself.
...
func (s *Server) handleToolsCall(ctx context.Context, req jsonrpc.Request) error {
	...
	// Rich-content path: handler returned a RawResult, take its content
	// blocks verbatim.
