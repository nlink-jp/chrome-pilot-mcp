...
func TestBasicRoundTrip(t *testing.T) {
	...
	srv := New("chrome-pilot-mcp", "test", tr, slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	if !strings.Contains(lines[0], `"name":"chrome-pilot-mcp"`) {
...
// TestStructuredToolError checks that a *toolerr.Error surfaces as isError
// with the structured {code, message} JSON in the text block.
func TestStructuredToolError(t *testing.T) {
	...
	srv.RegisterTool(Tool{
		...
		return nil, toolerr.New(toolerr.CodePageNotFound, "no page at index 3")
	})
	...
	if !strings.Contains(s, `page_not_found`) {
		t.Errorf("expected structured code page_not_found; got: %s", s)
	}
}

// TestRawResultContent checks the rich-content path: a handler returning
// RawResult passes its content blocks (e.g. an image) through verbatim.
func TestRawResultContent(t *testing.T) {
	in := bytes.NewBufferString(
		`{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":"shot","arguments":{}}}` + "\n")
	var out bytes.Buffer
	tr := transport.NewStdioTransport(in, &out)
	srv := New("chrome-pilot-mcp", "test", tr, slog.New(slog.NewTextHandler(io.Discard, nil)))
	...
	srv.RegisterTool(Tool{
		...
		return RawResult{Content: []ContentBlock{
			{Type: "text", Text: `{"path":"/ws/shot.png"}`},
	...
	s := out.String()
	for _, want := range []string{`"type":"image"`, `"mimeType":"image/png"`, `"data":"aGVsbG8="`, `/ws/shot.png`} {
		if !strings.Contains(s, want) {
			t.Errorf("raw result missing %q: %s", want, s)
		}
	}
}
