...
- 残る限界: 付与した `work_dir` の中の HTML の JS は、その HTML 自身の内容を外へ送れる（ADR-0001 の egress の
  限界と同じ）。まだアタッチしていないタブには割り込みが効かないが、読み取りの入り口で止まる。付与した HTML が
  起こす worker は測っていない（割り込みはページのセッションにだけ入る）。`chrome:`・`devtools:`・`blob:`・
  `filesystem:` はこれまでどおりブラウザに任せる（任意のパスを読むものではない）。
