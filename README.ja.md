...
- ファイルを産むツールはすべて**必須**の絶対パス `work_dir` を取り、その配下に
  ...
  `~` の展開も相対パスの解決もここでは行われません。システム領域、ホーム
...
- ローカルファイル（`file://`、`view-source:` の中のものも）は `work_dir` の下のものだけ開きます。
  ...
  `_meta`。既定はありません）、その下でも資格情報・エージェント制御の位置は拒否します。そのタブが読み込める
  ローカルファイル（CSS や画像、フレーム、スクリプトによる移動）も、そのタブを開いたときの `work_dir` の下のもの
  だけで、それ以外は Chrome の中で止めます。ほかのローカルファイルを表示しているタブ（スクリプトが `window.open`
  で開いたもの、あなたが開いたもの）は `navigate_page` で離れることしかできず、そのファイルを表示しているあいだは
  コンソールメッセージ・ネットワークリクエスト・ダイアログの文言も返しません。`--attach` のときは、このサーバーが
  使ったタブであなたが手で開いたローカルファイルも同じく止まります。許可は `work_dir` の下すべてに及ぶので、
  いちばん狭いディレクトリを渡してください。
